...
#
# if torch.backends.mps.is_available():  # Check for MPS availability (Apple Silicon)
#     device = torch.device("mps")
# elif torch.cuda.is_available():  # Check for CUDA availability (Nvidia GPUs)
...
failed_videos = []
root = '/Users/valentineo/MLProjects/Dissertation/datasets/RAVDESS/videos/'
...
for sess in tqdm(sorted(os.listdir(root))):
    ...
    session_path = os.path.join(root, sess)
    ...
    for filename in os.listdir(session_path):
        ...
        if filename.startswith('.'):  # Skip hidden files again
            continue
